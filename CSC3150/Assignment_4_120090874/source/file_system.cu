...
__device__ u32 fs_write(FileSystem *fs, uchar* input, u32 size, u32 fp)
{
  ...
  if (size <= 32){    //因为我们也给空文件分配了一个block的，所以小于32byte的内容可以塞得进去
    ...
    //当前block_index位置不够啊，需要compact
      ...
      //更新vcb
      // int new_start = new_index; 
      // for(int i = 0; i < block_need; i++){    //更新vcb这个bitmap
      //   int byte_3 = new_start / 8;
      //   int off_3 = new_start % 8;   //start是当前文件的stock_index
      //   fs->volume[byte_3] |= 1 << offset;   //更新VCB这个bitmap
      //   new_start += 1;
      // }
  ...
  // int b_1 = fs->volume[4096 + 32 * FCB_index + 29];
  // int b_2 = fs->volume[4096 + 32 * FCB_index + 30];
  // int b_3 = fs->volume[4096 + 32 * FCB_index + 31];
  // int sizes= b_1 * 65536 + b_2 * 256 + b_3;
  // printf("FCB_index is %d, size is %d\n",FCB_index, sizes);
  //printf("FCB_index is %d, original_block_index is %d, start_block_index is %d\n", FCB_index, block_index, new_index);
  return 0;
  // }
...
__device__ void fs_gsys(FileSystem *fs, int op)
{
  ...
  if (op == LS_D){
    ...
    for (int i = 0; i < index; i++){  //据说不要一个一个字符打
      ...
      printf("%s\n", name);
      // modified_time_1 = 0;
      // modified_time_1 += (fs->volume[4096 + 32 * arr[i] + 26]) * 256;
      // modified_time_1 += fs->volume[4096 + 32 * arr[i] + 27];
      // printf("%d\n",modified_time_1);
    }
